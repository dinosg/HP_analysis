...
#find out when Mystic is dispatched - the Mystic facility but only SOME of the units
ismysticgas = (epa_2018['Facility ID'] == "1588") & (epa_2018['Primary Fuel Type'] == 'Gas') & ((epa_2018['Unit ID'] == "81") | (epa_2018['Unit ID'] == "82") \
                                                                                             | (epa_2018['Unit ID'] == "93" )|( epa_2018['Unit ID'] == "94")) 
...
#************************  HEATRATES USING EPA calced heatrate
#this is JUNK because the heatrate info from the EPA fuel and power generation info is UNRELIABLE
"""
heatrates = epa_nocogenf[heatratecols1]
marg_hr = heatrates.groupby(['Date', 'Hour', "inferred_fuel" ]).max()
gas_marghr = marg_hr.query("inferred_fuel== 'Gas' ")



marg_gasplants = gas_marghr["Facility Name"].unique()
marg_gasplants_hr = gas_marghr.pivot_table(columns="Facility Name", values='EPA_heatrate', aggfunc='max')
marg_gasplants_hrm = gas_marghr.pivot_table(columns="Facility Name", values='EPA_heatrate', aggfunc='max')
marg_gasplants_hr = marg_gasplants_hr.transpose()

marg_gasplants_hrm = marg_gasplants_hrm.transpose()
marg_hr_pivot = marg_hr.pivot_table(index=['Date', 'Hour', 'Facility Name'], columns= 'inferred_fuel', values='EPA_heatrate', aggfunc='sum')
#marg_hr_pivot = marg_hr.pivot_table(index=['Date', 'Hour'], columns= 'inferred_fuel', values='EPA_heatrate', aggfunc='sum')  """
...
max_indices = grouped_df['heatrate'].idxmax()
...
h1 = list(marg_gas['heatrate'])
h2 = list(marg_oil['heatrate'])
h3 = list(marg_resid['heatrate'])
...
heatrates["power_cost"] = heatrates["fuel_cost"] *heatrates["heatrate"]/1000
heatrates.loc[heatrates["power_cost"].isna(), "power_cost"] = 0

heatrates.loc[isgas,"emissions"] = gas_CO2 * heatrates.loc[isgas,"heatrate"]/1000
heatrates.loc[iscoal,"emissions"] = coal_CO2 * heatrates.loc[iscoal,"heatrate"]/1000
heatrates.loc[isresid,"emissions"] = resid_CO2 * heatrates.loc[isresid,"heatrate"]/1000
heatrates.loc[isdiesel,"emissions"] = diesel_CO2 * heatrates.loc[isdiesel,"heatrate"]/1000
heatrates.loc[isdiesel,"emissions"] = diesel_CO2 * heatrates.loc[isdiesel,"heatrate"]/1000
heatrates.loc[iswood,"emissions"] = wood_CO2 * heatrates.loc[iswood,"heatrate"]/1000
...
effective_emissionsavg = effective_emissionsavg.sum()
overall_gen = epa_2018i.groupby(['Date', 'Hour']).sum() #most of the columns will be junk but 'MWh' will be interesting usable data

TDlosses = 1.06
efficiency_comparison = marg_plants_nofuel['COP'] * (1/TDlosses)  * 3412.14/marg_plants_nofuel['heatrate']
...
eff_emissions_comparison_int = TDlosses * marg_plants_nofuel['emissions'] * marg_plants_nofuel['heatload'] / marg_plants_nofuel['COP'] /(gas_CO2*3.41214)
